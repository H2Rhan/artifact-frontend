...
(async () => {
  ...
  try {
    ...
    console.error(' 预加载失�?', error.message);
...
async function scrapeTrendingGoods() {
  ...
  try {
    ...
    console.log(`  微拍�? ${wptGoods.length} 个商品`);
  } catch (error) {
    console.error('  微拍堂爬取失�?', error.message);
  }
...
async function scrapeWeiPaiTang() {
  ...
  try {
    ...
    console.error('  微拍堂解析失�?', error.message);
  }
...
function getCategory(name) {
  ...
  if (name.includes('银元') || name.includes('袁大�?) || name.includes('光绪')) {
    return '银元';
  }
  if (name.includes('刀�?) || name.includes('布币')) {
    return '青铜�?;
  }
  if (name.includes('纪念�?) || name.includes('纪念')) {
    return '纪念�?;
  }
  return '古钱�?;
}
...
async function axiosWithRetry(url, config, retries = MAX_RETRIES) {
  try {
    ...
    if (retries > 0 && (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED')) {
      console.log(`�?请求超时�?{RETRY_DELAY / 1000}秒后重试... (剩余 ${retries} �?`);
...
app.listen(PORT, () => {
  ...
  console.log('�?蓝心大模型代理服务器运行�?http://localhost:8000');
});
